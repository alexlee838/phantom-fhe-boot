...
uint32_t Degree(const std::vector<double>& coefficients) {
    ...
    if (!coefficientsSize) {
        throwError("The coefficients vector can not be empty");
    }
...
std::shared_ptr<longDiv> LongDivisionPoly(const std::vector<double>& f, const std::vector<double>& g) {
    ...
    if (n != f.size() - 1) {
        throwError("LongDivisionPoly: The dominant coefficient of the divident is zero.");
    }

    if (k != g.size() - 1) {
        throwError("LongDivisionPoly: The dominant coefficient of the divisor is zero.");
    }
...
std::shared_ptr<longDiv> LongDivisionChebyshev(const std::vector<double>& f, const std::vector<double>& g) {
    ...
    if (n != f.size() - 1) {
        throwError("LongDivisionChebyshev: The dominant coefficient of the divident is zero.");
    }

    if (k != g.size() - 1) {
        throwError("LongDivisionChebyshev: The dominant coefficient of the divisor is zero.");
    }
...
std::vector<uint32_t> ComputeDegreesPS(const uint32_t n) {
    if (n == 0) {
        throwError("ComputeDegreesPS: The degree is zero. There is no need to evaluate the polynomial.");
    }
...
uint32_t GetMultiplicativeDepthByCoeffVector(const std::vector<double>& vec, bool isNormalized) {
    ...
    if (!vecSize) {
        throwError("Cannot perform operation on empty vector. vec.size() == 0");
    }
...
std::vector<cuDoubleComplex> Rotate(const std::vector<std::complex<double>>& a, int32_t index) {
    ...
    std::vector<cuDoubleComplex> result(slots);

    // Ensure index is within the valid range
    if (index < 0 || index > slots) {
        index = ReduceRotation(index, slots);  // Assuming ReduceRotation exists
    }

    if (index == 0) {
        for (int32_t i = 0; i < slots; i++) {
            result[i] = make_cuDoubleComplex(a[i].real(), a[i].imag());
        }
    } else {
        // Rotation logic with proper indexing
        for (int32_t i = 0; i < slots - index; i++) {
            result[i] = make_cuDoubleComplex(a[i + index].real(), a[i + index].imag());
        }
        for (int32_t i = slots - index; i < slots; i++) {
            result[i] = make_cuDoubleComplex(a[i + index - slots].real(), a[i + index - slots].imag());
        }
    }
...
std::vector<int32_t> FindLTRotationIndicesSwitchArgmin(uint32_t m, uint32_t blockDimension, uint32_t cols) {
        ...
        if (i == int32_t(m - 1)) {
            ...
            if (i < 0) {
                if(!phantom::arith::try_invert_uint_mod(5, m, g0)) {
                    throwError("Invalid in FindAutomorphism");
                }
            }
            else {
                g0 = 5;
    ...
    __global__ void PrecomputeAutoMapKernel(uint32_t n, uint32_t k, uint32_t* precomp) {
        ...
        // Grid-stride loop: ensures all elements are computed even if not enough threads
        for (uint32_t j = threadIdx.x + blockIdx.x * blockDim.x;
            ...
            uint32_t jrev    = reverse_bits_aux(j, logn);
            uint32_t idxrev  = reverse_bits_aux(idx, logn);
